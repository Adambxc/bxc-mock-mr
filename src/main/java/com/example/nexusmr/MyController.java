...
public class MyController {
  ...
  public List<User> getUsers() throws URISyntaxException, JsonProcessingException {
    ...
    var result = restTemplate.exchange(getUsersUrl, HttpMethod.GET, httpEntity, new ParameterizedTypeReference<List<OktaUser>>() {});
    var oktaUsers = result.getBody();
    List<User> users = new ArrayList<>();
    for (int i = 0; i < oktaUsers.size(); i++) {
      OktaUser oktaUser = oktaUsers.get(i);
      var groups = getUsersGroups(oktaUser.id);
      var groupIds = objectMapper.writeValueAsString(groups.stream().map(OktaGroup::getId).collect(Collectors.toList()));
  ...
  public List<OktaGroup> getUsersGroups(String userId) {
    ...
    var result = restTemplate.exchange(url, HttpMethod.GET, httpEntity, new ParameterizedTypeReference<List<OktaGroup>>() {});


    return (List<OktaGroup>) result.getBody();
  }
}
